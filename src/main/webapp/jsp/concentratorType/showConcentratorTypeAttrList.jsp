...
<script type="text/javascript">
		...
		if (rows.length > 0) {
			$.messager.confirm('确认', '你确定要移除这个信息码?', function(r) {
				if (r) {
					...
					$.post('../concentratorTypeAttr/destroys.do', {
	...
	function accept() {
		...
		if (inserted.length > 0) {
			url = '../concentratorTypeAttr/save.do';
		}
		if (updated.length > 0) {
			url = '../concentratorTypeAttr/update.do';
		}
...
<table id="dgSon" title="类型属性" class="easyui-datagrid"
	...
	data-options="
					...
					singleSelect: false,
					toolbar: '#tb',
					url: '../concentratorTypeAttr/getListJson.do?conTypeId=${conTypeId}',
					method: 'get',
					pagination: true,
					rownumbers: true,
					fitColumns: true">
	<thead>
		<tr>
			...
			<th data-options="field:'portName',width:80,align:'center',editor:'textbox'">端口名称</th>
			<th field="portCode" width="80" align="center" editor="textbox">默认编码</th>
			<th field="conTypeId" hidden="true"></th>
